...
class DCGAN():
    def __init__(self):
        ...
        self.batch_per_epoch = int(self.X_train.shape[0] / batch_size)
    ...
    def generate_latent_points(self, latent_dimension, n_samples):
        # Generate points in the latent space
        input = np.random.randn(latent_dimension * n_samples)

        # Reshape into a batch of inputs for the network
        input = input.reshape(n_samples, latent_dimension)
    ...
    def build_generator(self):
        ...
        model.add(Dense(units = 128 * 4 * 4, input_dim = self.latent_dimension))
        ...
        model.add(Conv2DTranspose(filters = 128, kernel_size = (4, 4), strides = (2, 2), padding = 'same'))
        model.add(LeakyReLU(alpha = 0.2))
        model.add(Conv2D(filters = self.channels, kernel_size = (5, 5), strides = (1, 1), activation = 'tanh', padding = 'same'))
    ...
    def build_discriminator(self):
        ...
        model.add(Conv2D(128, kernel_size = (5, 5), strides = (2, 2), input_shape = (self.height, self.width, self.channels), padding = 'same'))
        model.add(LeakyReLU(alpha = 0.2))
    ...
    def train(self, epochs, batch_size, save_interval):
        ...
        for k in range(1, epochs + 1):
            for l in tqdm(range(self.batch_per_epoch)):
                ...
                print ('\nTraining epoch : %d \nTraining batch : %d \nAccuracy of discriminator : %.2f%% \nLoss of discriminator : %f \nLoss of generator : %f ' 
                        % (k, l, discriminator_loss * 100, discriminator_loss, generator_loss))

                record = (k, l, discriminator_loss * 100, discriminator_loss, generator_loss)
